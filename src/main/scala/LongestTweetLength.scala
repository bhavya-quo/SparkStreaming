...
/** Uses thread-safe counters to keep track of the longest length of
 *  Tweets in a stream.
 */
...
//    val lengths = statuses.map(status => status.length())
    ...
    statuses.foreachRDD((statusRDD) => {
        statusRDD.foreach(status => {
          ...
          if (longestTweetLength.get() < length) {
            longestTweetLength.set(length)
          }
          println(longestTweetLength)
        })
